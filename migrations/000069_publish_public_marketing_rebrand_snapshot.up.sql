-- Republish the platform's public-marketing landing page snapshot so the Zyad Technovation
-- rebrand + legal footer links (000068) are served by the public resolve endpoint, which
-- reads from the latest landing_page_versions snapshot rather than live
-- landing_page_sections rows once a page has been published at least once.
...
SELECT
	...
	lp.id,
	nv.version,
	'Seed: republish with Zyad Technovation legal footer',
	jsonb_build_object(
