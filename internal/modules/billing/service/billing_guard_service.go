...
type BillingGuardSubscriptionStore interface {
	FindUsableByOrganization(ctx context.Context, organizationID string) (billingmodel.Subscription, error)
	List(ctx context.Context, filter repository.SubscriptionListFilter) ([]billingmodel.Subscription, int64, error)
}

type BillingGuardEntitlementEvaluator interface {
...
type BillingGuardService struct {
	subscriptions BillingGuardSubscriptionStore
	entitlements  BillingGuardEntitlementEvaluator
	now           func() time.Time
}

func NewBillingGuardService(
	subscriptions BillingGuardSubscriptionStore,
	entitlements BillingGuardEntitlementEvaluator,
) *BillingGuardService {
	return &BillingGuardService{
...
func (s *BillingGuardService) RequireFeature(
	ctx context.Context,
	organizationID string,
	featureKey string,
...
func (s *BillingGuardService) RequireQuota(ctx context.Context, input QuotaInput) (organizationmodel.Entitlement, error) {
	if input.Delta <= 0 {
		input.Delta = 1
	}
	...
	if limit != nil && input.UsedValue+input.Delta > *limit {
		return organizationmodel.Entitlement{}, billing.QuotaExceededError()
	}
	return entitlement, nil
}

func (s *BillingGuardService) RequireQuotaValue(
	ctx context.Context,
	organizationID string,
	featureKey string,
	limitKey string,
...
func (s *BillingGuardService) requireUsableSubscription(ctx context.Context, organizationID string) error {
	...
	subscription, err := s.subscriptions.FindUsableByOrganization(ctx, organizationID)
	if err == nil && subscription.IsUsable() {
		return nil
	}
	...
	case billingmodel.SubscriptionStatusSuspended:
		return billing.SubscriptionSuspendedError()
	case billingmodel.SubscriptionStatusCanceled,
		billingmodel.SubscriptionStatusExpired:
		return billing.SubscriptionInactiveError()
	default:
		return billing.SubscriptionNotFoundError()
	}
}

func (s *BillingGuardService) latestSubscription(ctx context.Context, organizationID string) (billingmodel.Subscription, error) {
	subscriptions, _, err := s.subscriptions.List(ctx, repository.SubscriptionListFilter{
		OrganizationID: strings.TrimSpace(organizationID),
		Limit:          1,
	})
	if err != nil {
		return billingmodel.Subscription{}, err
	}
	if len(subscriptions) == 0 {
		return billingmodel.Subscription{}, billing.SubscriptionNotFoundError()
	}
	return subscriptions[0], nil
}

func (s *BillingGuardService) requireEntitlement(
	ctx context.Context,
	organizationID string,
	featureKey string,
) (organizationmodel.Entitlement, error) {
	...
	if err != nil {
		...
		if errors.As(err, &appErr) && appErr.Code == "ORGANIZATION_FEATURE_NOT_ENTITLED" {
			return organizationmodel.Entitlement{}, billing.FeatureNotEnabledError()
		}
		return organizationmodel.Entitlement{}, err
	}
	return entitlement, nil
