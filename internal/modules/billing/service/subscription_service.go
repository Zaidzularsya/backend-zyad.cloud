...
func (s *SubscriptionService) FindByID(ctx context.Context, organizationID string, id string) (dto.SubscriptionResponse, error) {
	subscription, err := s.store.FindByID(ctx, strings.TrimSpace(organizationID), strings.TrimSpace(id))
	if err != nil {
		return dto.SubscriptionResponse{}, mapSubscriptionError(err)
	}
	return subscriptionResponse(subscription, nil), nil
}
...
func (s *SubscriptionService) FindUsableByOrganization(ctx context.Context, organizationID string) (dto.SubscriptionResponse, error) {
	subscription, err := s.store.FindUsableByOrganization(ctx, strings.TrimSpace(organizationID))
	if err != nil {
		return dto.SubscriptionResponse{}, mapSubscriptionError(err)
	}
	return subscriptionResponse(subscription, nil), nil
...
func (s *SubscriptionService) FindLatestByOrganization(ctx context.Context, organizationID string) (dto.SubscriptionResponse, error) {
	...
	if len(subscriptions) == 0 {
		return dto.SubscriptionResponse{}, billing.SubscriptionNotFoundError()
	}
	return subscriptionResponse(subscriptions[0], nil), nil
}
...
func (s *SubscriptionService) Create(ctx context.Context, request dto.CreateSubscriptionRequest) (dto.SubscriptionResponse, error) {
	...
	subscription, err := s.store.Create(ctx, params)
	if err != nil {
		return dto.SubscriptionResponse{}, err
	}
	if _, err := s.store.CreateEvent(ctx, repository.SubscriptionEventParams{
		...
		Metadata:       map[string]any{"plan_id": subscription.PlanID},
	}); err != nil {
		return dto.SubscriptionResponse{}, err
	}
	if subscription.IsUsable() {
		if err := s.syncEntitlements(ctx, subscription, "", "subscription created"); err != nil {
			return dto.SubscriptionResponse{}, err
		}
	}
	return subscriptionResponse(subscription, nil), nil
...
func (s *SubscriptionService) updateWithActor(
	...
	return subscriptionResponse(updated, nil), nil
}
...
func (s *SubscriptionService) UpdateByID(
	...
	subscription, err := s.store.FindByIDUnscoped(ctx, strings.TrimSpace(id))
	if err != nil {
		return dto.SubscriptionResponse{}, mapSubscriptionError(err)
	}
	return s.Update(ctx, subscription.OrganizationID, subscription.ID, request)
}
...
func (s *SubscriptionService) ChangeStatusByID(
	...
	return s.ChangeStatus(ctx, subscription.OrganizationID, subscription.ID, status, actorUserID, reason)
}
...
func (s *SubscriptionService) ScheduleCancellation(
	...
	if current.CancelAtPeriodEnd {
		return subscriptionResponse(current, nil), nil
	}
...
func (s *SubscriptionService) syncEntitlements(
	...
	entitlements, err := s.entitlementStore.ListByPlanID(ctx, subscription.PlanID)
	if err != nil {
		return err
	}
	effectiveFrom := s.now().UTC()
	if subscription.CurrentPeriodStart != nil {
		effectiveFrom = subscription.CurrentPeriodStart.UTC()
	}
...
func (s *SubscriptionService) expireEntitlements(
	...
	if subscription.CurrentPeriodEnd != nil {
		effectiveUntil = subscription.CurrentPeriodEnd.UTC()
	}
...
func upgradeActivationReason(invoice model.Invoice) string {
