...
type PlanBenefitStore interface {
	ListActiveByPlanID(ctx context.Context, planID string) ([]repository.PublicEntitlement, error)
}

type PlanService struct {
	store        PlanStore
	entitlements PlanBenefitStore
}

func NewPlanService(store PlanStore, entitlements PlanBenefitStore) *PlanService {
	return &PlanService{store: store, entitlements: entitlements}
}
...
func (s *PlanService) ListPublic(ctx context.Context) (dto.PublicPlanListResponse, error) {
	isPublic := true
	isActive := true
	plans, _, err := s.store.List(ctx, repository.PlanListFilter{
		IsPublic: &isPublic,
		IsActive: &isActive,
	})
	if err != nil {
		return dto.PublicPlanListResponse{}, err
	}
	items := make([]dto.PublicPlanSummary, 0, len(plans))
	for _, plan := range plans {
		prices, err := s.store.ListPrices(ctx, plan.ID, false)
		if err != nil {
			return dto.PublicPlanListResponse{}, err
		}
		benefits, err := s.entitlements.ListActiveByPlanID(ctx, plan.ID)
		if err != nil {
			return dto.PublicPlanListResponse{}, err
		}
		items = append(items, publicPlanSummary(plan, prices, benefits))
	}
	return dto.PublicPlanListResponse{Items: items}, nil
}
