...
func (r *PlanEntitlementRepository) ListByPlanID(ctx context.Context, planID string) ([]model.PlanEntitlement, error) {
	rows, err := r.db.Query(ctx, `
		...
		FROM product_plan_entitlements entitlement
		JOIN product_features feature ON feature.id = entitlement.feature_id
...
func (r *PlanEntitlementRepository) ListActiveByPlanID(ctx context.Context, planID string) ([]PublicEntitlement, error) {
	rows, err := r.db.Query(ctx, `
		SELECT
			feature.feature_key,
			feature.name,
			COALESCE(feature.unit, ''),
			feature.value_type,
			entitlement.value_bool,
			entitlement.value_int,
			entitlement.value_decimal::text,
	...
	entitlements := make([]PublicEntitlement, 0)
	for rows.Next() {
		var item PublicEntitlement
		if err := rows.Scan(
			&item.FeatureKey,
			&item.FeatureName,
			&item.Unit,
